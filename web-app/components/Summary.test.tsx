...
describe("Summary", () => {
	...
	it("shows average", () => {
		...
		const memberJohnDoe = createRoomMember({username: "John Doe", vote: card1});
		const memberAlice = createRoomMember({username: "Alice", vote: card5});
		const room = createRoom({cardSetName: cardSet.name, votingClosed: true, members: [memberJohnDoe, memberAlice]});
	...
	it("formats average", () => {
		const card1 = createCard({value: 1});
		const card5 = createCard({value: 5});
		const cardSet = createCardSet({cards: [card1, card5], relevantFractionDigits: 0});
