	...
	describe("getProductScoreFactNames", () => {
		...
		it("shows button", async () => {
			vi.mocked(ahaClient.getIdeasForProduct<"score_facts">).mockResolvedValue({
				ideas: [
					{
						id: "111", product_id: "456", score_facts: []
					},
					{
						id: "222", product_id: "456", score_facts: [{
							name: "Foo",
							value: 0
						}]
					},
					{
						id: "333", product_id: "456", score_facts: [{
							name: "Foo",
							value: 0
						}, {
							name: "Bar",
			...
			expect(ahaClient.getIdeasForProduct).toHaveBeenCalledWith("456", 1, 200, ["score_facts"]);
		});
	});
});